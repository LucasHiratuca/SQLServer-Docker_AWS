...
A pasta `outputs/` documenta cada etapa com screenshots reais de execução, provando que o ambiente funciona de ponta a ponta.
...
A tabela `Treino_Exercicio` mantém os campos `Carga` e `Reps` intencionalmente. Esses valores são **atributos do relacionamento em si** — não pertencem ao Treino isoladamente, nem ao Exercício isoladamente, mas à combinação dos dois. Não existe forma de derivar via SELECT que um aluno fez supino com 80kg e 10 repetições em determinado treino — esse dado precisa ser registrado. Removê-los significaria perda irreversível de informação.
...
Esta seção registra todos os ajustes aplicados durante a revisão do MER e DER. O modelo foi revisado e **aprovado** na versão atual.
