...
# Plano
# ============================================
planos = [('Basic', 30), ('Standard', 90), ('Premium', 180), ('Anual', 365)]
...
for nome, duracao in planos:
    ...
    """, nome, nome, duracao)
conn.commit()
print(f"✅ Plano: {len(planos)} registros")
...
for _ in range(10):
    ...
    try:
        ...
        pass
print(f"✅ Personal: {len(personals)} registros")
...
for _ in range(5):
    ...
    try:
        ...
        pass
print(f"✅ Professor: {len(professores)} registros")
...
for _ in range(50):
    ...
    cpf_personal = random.choice(personals) if personals else None
    ...
    try:
        ...
        pass
print(f"✅ Aluno: {len(alunos)} registros")

# ============================================
# Aula
# ============================================
aulas_disponiveis = ['Musculacao', 'Yoga', 'Spinning', 'Pilates', 'Funcional']
aulas = []

for nome_aula in aulas_disponiveis:
    cpf_professor = random.choice(professores) if professores else None
    try:
        cursor.execute("""
        ...
        pass
print(f"✅ Aula: {len(aulas)} registros")
...
for aluno in alunos:
    aulas_aluno = random.sample(aulas, k=min(random.randint(1, 3), len(aulas)))
    for aula in aulas_aluno:
        ...
        if par not in pares_aluno_aula:
            try:
                ...
                pass
print(f"✅ Aluno_Aula: {aluno_aula_inseridos} registros")
...
for tipo, nome, preco in produtos_disponiveis:
    ...
    id_produto = cursor.fetchval()
    ids_produto.append(id_produto)

print(f"✅ Produto: {len(ids_produto)} registros")
...
for aluno in random.sample(alunos, k=min(30, len(alunos))):
    produtos_aluno = random.sample(ids_produto, k=min(random.randint(1, 3), len(ids_produto)))
    for id_produto in produtos_aluno:
        par = (id_produto, aluno)
        if par not in pares_produto_aluno:
            try:
                cursor.execute("""
                    INSERT INTO Produto_Aluno (ID_Produto, CPF_Aluno, Quantidade_Comprada)
                    VALUES (?, ?, ?)
                ...
                pass
print(f"✅ Produto_Aluno: {produto_aluno_inseridos} registros")

# ============================================
# Exercicio
# ============================================
exercicios_disponiveis = [
    ('Supino Reto', 'Peito'), ('Supino Inclinado', 'Peito'),
    ('Agachamento', 'Pernas'), ('Leg Press', 'Pernas'),
    ('Remada Curvada', 'Costas'), ('Puxada Frontal', 'Costas'),
    ('Desenvolvimento', 'Ombros'), ('Elevacao Lateral', 'Ombros'),
    ('Rosca Direta', 'Biceps'), ('Triceps Pulley', 'Triceps'),
    ('Panturrilha', 'Pernas'), ('Abdominal', 'Core'),
]
...
for nome, parte in exercicios_disponiveis:
    ...
    """, nome, nome, parte)
    conn.commit()
print(f"✅ Exercicio: {len(exercicios_disponiveis)} registros")
...
for aluno in alunos:
    ...
    for _ in range(num_treinos):
        ...
        cursor.execute("SELECT @@IDENTITY")
        id_treino = cursor.fetchval()
        treinos.append(id_treino)

print(f"✅ Treino: {len(treinos)} registros")
...
for id_treino in treinos:
    exercicios_treino = random.sample(nomes_exercicio, k=min(random.randint(3, 6), len(nomes_exercicio)))
    for exercicio in exercicios_treino:
        ...
        try:
            ...
            pass
print(f"✅ Treino_Exercicio: {treino_exercicio_inseridos} registros")
